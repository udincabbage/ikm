...
WHERE tresponden_kuis.responden="DOSEN"
GROUP by nama_aspek';
...
<form id="new-project"  action="?page=Kuesioner-Dosen" method="post" name="form1" target="_self">
	<table class="table table-bordered table-striped">
		<tr>
			<td colspan="2"><h1><b>Kuesioner Dosen</b></h1></td>
		</tr>
		...
		<table class="table table-bordered table-striped">
			...
			while ($kolomData = mysqli_fetch_array($myQry)) {
				...
				LEFT JOIN tkuis ON tresponden_kuis.id_kuis=tkuis.id_kuis
				LEFT JOIN taspek ON tkuis.id_aspek = taspek.id_aspek
				WHERE tresponden_kuis.responden="DOSEN" and tkuis.id_aspek='.$Kode;
				...
				<?php $nomor2=1;while ($kolomData2 = mysqli_fetch_array($queryKuis)) { ?>
				<tr>
					<td><strong> <?php echo $nomor2 ?> </strong></td>
					<td><strong> <?php echo $kolomData2[$field2]; ?> </strong></td>
					<td colspan="2"><label for="<?php echo $kolomData2[$field0]; ?> "> <input type="radio" name="<?php echo $kolomData2[$field0]; ?> " value="a" required > <?php echo $kolomData2[$field3]; ?></label></td>

					<td colspan="2"><label for="<?php echo $kolomData2[$field0]; ?> "> <input type="radio" name="<?php echo $kolomData2[$field0]; ?> " value="b"> <?php echo $kolomData2[$field4]; ?></label></td>

					<td colspan="2"><label for="<?php echo $kolomData2[$field0]; ?> "> <input type="radio" name="<?php echo $kolomData2[$field0]; ?> " value="c"> <?php echo $kolomData2[$field5]; ?></label></td>

					<td colspan="2"><label for="<?php echo $kolomData2[$field0]; ?> "> <input type="radio" name="<?php echo $kolomData2[$field0]; ?> " value="d"> <?php echo $kolomData2[$field6]; ?></label></td>
				</tr>
				<?php $nomor2++;} ?>
	...
	<table class="table table-bordered table-striped">
		...
		for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Kuesioner-Dosen&hal=$list[$h]'>$h</a> ";
	}
	?>	</td>
</tr> -->
<tr>
