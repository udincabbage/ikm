...
if($_GET) {
	...
	if(isset($_POST['btnSave'])){
			...
			{   
				...
				// {
				   $mySql="INSERT INTO tjawaban (ip_address, tahun, bulan, id_kuesioner, id_kuis, jawaban)
							   VALUES ('$user_ip', '$tahun', '$bulan', '$id_kuesioner', '$key', '$val')";
				
				// }
				$myQry	= mysqli_query($koneksidb, $mySql) or die ("Gagal query".mysql_error());
			
			$mySql = "DELETE FROM tjawaban WHERE  id_kuis='0'";
			$myQry	= mysqli_query($koneksidb, $mySql) or die ("Gagal buang 0 ".mysql_error());
			if($myQry){
				
			$_SESSION['TEST'] ='1';
				echo "<meta http-equiv='refresh' content='0; url=?page=Kuesioner-Selesai'>";
...
<table class="table table-bordered table-striped">
  <tr>
	...
	<table class="table table-striped">
      ...
      <tr>
        <td colspan="2"><label for="<?php echo $kolomData[$field1]; ?> "> <input type="radio" name="<?php echo $kolomData[$field1]; ?> " value="a"> <?php echo $kolomData[$field3]; ?></label></td>
      </tr>
      <tr>
        <td colspan="2"><label for="<?php echo $kolomData[$field1]; ?> "> <input type="radio" name="<?php echo $kolomData[$field1]; ?> " value="b"> <?php echo $kolomData[$field4]; ?></label></td>
      </tr>
      <tr>
        <td colspan="2"><label for="<?php echo $kolomData[$field1]; ?> "> <input type="radio" name="<?php echo $kolomData[$field1]; ?> " value="c"> <?php echo $kolomData[$field5]; ?></label></td>
      </tr>
      <tr>
        <td colspan="2"><label for="<?php echo $kolomData[$field1]; ?> "> <input type="radio" name="<?php echo $kolomData[$field1]; ?> " value="d"> <?php echo $kolomData[$field6]; ?></label></td>
      </tr>
