...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if (count($pesanError)>=1 ){
			...
			$tmpSql ="SELECT tjawaban_temp.id_kuis,sum(nilai) AS bobot, sum(nilai)/ COUNT(tjawaban_temp.id_kuis) AS unsur, tkuis.jawaban_c, tkuis.jawaban_d, tkuis.jawaban_b, tkuis.jawaban_a,taspek.id_aspek,taspek.nama_aspek FROM tjawaban_temp LEFT JOIN tkuis ON tkuis.id_kuis=tjawaban_temp.id_kuis LEFT JOIN taspek ON taspek.id_aspek=tkuis.id_aspek WHERE tahun='$txt1' AND bulan='$txt5' GROUP BY tjawaban_temp.id_kuis";
